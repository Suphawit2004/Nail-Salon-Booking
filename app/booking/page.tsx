...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return <h2 className="text-sm font-semibold text-pink-600 mb-3 select-none">{children}</h2>;
}
    ...
    <div className="rounded-3xl border border-pink-100 shadow-sm bg-white/80 backdrop-blur-sm p-3 sm:p-4 flex gap-3">
      <div className="shrink-0">
        <div className="relative w-[88px] h-[88px] sm:w-[96px] sm:h-[96px] rounded-2xl bg-pink-50 border border-pink-100 overflow-hidden">
          {service.thumbnail ? <Image src={service.thumbnail} alt={service.title} fill className="object-cover" /> : null}
        </div>
      </div>
      <div className="flex-1">
        ...
        <div className="rounded-2xl bg-white border border-pink-100 p-3 text-[13px] leading-relaxed text-gray-600">
          ...
          {service.note ? (<div className="text-[12px] text-gray-400 mt-2">{service.note}</div>) : null}
        </div>
        <div className="mt-2 flex justify-end gap-2">
          <Link href={`/reserve/${service.id}`} className="inline-flex items-center justify-center rounded-xl px-4 py-2 text-xs font-semibold text-white bg-pink-400 hover:bg-pink-500 active:bg-pink-600 transition-colors shadow-sm">
            จอง
          </Link>
        </div>
      </div>
