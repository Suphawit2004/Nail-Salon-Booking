...
export default function PaymentPage() {
  ...
  if (loading) return <main className="mx-auto max-w-screen-sm min-h-screen bg-white p-6">กำลังโหลด…</main>;
  if (!booking) return <main className="mx-auto max-w-screen-sm min-h-screen bg-white p-6">ไม่พบข้อมูลการจอง</main>;
      ...
      <section className="px-4 pt-4 pb-28">
        ...
        <div className="rounded-3xl border border-pink-100 bg-pink-50/60 p-4 shadow-sm">
          <div className="flex gap-3">
            <div className="relative w-[100px] h-[100px] rounded-2xl bg-pink-50 border border-pink-100 overflow-hidden" />
        ...
        <div className="mt-5">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="rounded-2xl border border-pink-100 p-3">
              ...
              <div className="mt-2 text-xs text-gray-500">อัปโหลดสลิป (mock)</div>
              <input type="file" className="mt-1 text-xs" />
            </div>
            <div className="rounded-2xl border border-gray-200 p-3 text-sm text-gray-500">
              (เพิ่มช่องทางอื่นได้ เช่น PromptPay/QR)
            </div>
          </div>
        </div>
