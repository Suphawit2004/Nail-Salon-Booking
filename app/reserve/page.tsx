...
export default function ReservePage(){
  ...
  return (
    <LayoutWrapper>
      <h2 className="text-sm font-semibold text-pink-600 mb-3 mt-4">เลือกวัน เวลา</h2>
      ...
      <div className="rounded-3xl border border-pink-100 shadow-[0_6px_14px_rgba(255,182,193,0.25)] bg-pink-50/60 p-4">
        ...
        <div className="mt-4">
          ...
          <div className="grid grid-cols-4 sm:grid-cols-6 gap-2">
            {slots.map(t=>(
              <button key={t} type="button" onClick={()=>setTime(t)} className={
                "px-3 py-2 text-xs rounded-xl border transition " + (time===t ? "bg-pink-400 text-white border-pink-400" : "border-pink-200 hover:bg-pink-50")
              }>{t}</button>
            ))}
          </div>
        </div>
