...
/** Convert booking date/time (Thai) to JS Date */
function toStartDate(b: Booking) {
  return new Date(`${b.date}T${b.time}:00+07:00`);
}

/** Derive live status by comparing with current time (CANCELLED always wins) */
...
export default function AllBookingsPage() {
  ...
  useEffect(() => {
    ...
    es.onmessage = (evt) => {
      try {
        const data = JSON.parse(evt.data) as Booking[];
