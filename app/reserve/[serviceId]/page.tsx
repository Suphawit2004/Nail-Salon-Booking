...
export default function ReservePage(){
  ...
  const handleNext=()=>{
    ...
    router.push(`/reserve/${service.id}/confirm?date=${date}&time=${time}`);
  };

  if(!service) return <main className='mx-auto max-w-screen-sm min-h-screen bg-white p-6'>ไม่พบบริการ</main>;

  return (
    <main className='mx-auto min-h-screen max-w-screen-sm bg-white text-gray-800'>
      <header className='relative'><div className='h-36 bg-gradient-to-b from-pink-200 to-pink-50 flex flex-col items-center justify-center'><Image src='/logo.png' alt='logo' width={160} height={120} className='object-contain'/></div></header>
          ...
          <div className='flex gap-3'>
            ...
            <div className='flex-1'>
              ...
              <div className='rounded-2xl bg-white border border-pink-100 p-3 mt-1 text-[13px] text-gray-600'>
                <ul className='list-disc pl-5 space-y-0.5'>{service.bullets.map((b,i)=>(<li key={i}>{b}</li>))}</ul>
                {service.note && <div className='text-[12px] text-gray-400 mt-2'>{service.note}</div>}
              </div>
            </div>
          </div>
          <div className='grid sm:grid-cols-2 gap-4 mt-4'>
            <label className='flex flex-col'><span className='text-sm text-gray-600 flex items-center gap-2'><CalendarDays className='h-4 w-4 text-pink-500'/> วันที่</span><input type='date' className='border rounded-xl p-2 text-sm mt-1 focus:outline-pink-400' value={date} onChange={e=>setDate(e.target.value)}/></label>
            <label className='flex flex-col'><span className='text-sm text-gray-600 flex items-center gap-2'><Clock className='h-4 w-4 text-pink-500'/> เวลา</span><input type='time' className='border rounded-xl p-2 text-sm mt-1 focus:outline-pink-400' value={time} onChange={e=>setTime(e.target.value)}/></label>
          </div>
          <div className='mt-4 flex justify-end gap-2'>
            <Link href='/booking' className='px-4 py-2 rounded-xl border border-pink-200 text-sm hover:bg-pink-50'>ย้อนกลับ</Link>
            <button onClick={handleNext} className='rounded-xl bg-pink-400 text-white px-6 py-2 text-sm font-semibold hover:bg-pink-500 active:bg-pink-600 shadow-sm'>ถัดไป</button>
          </div>
        </div>
      </section>
    </main>
